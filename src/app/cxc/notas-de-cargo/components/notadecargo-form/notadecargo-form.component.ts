...
export class NotadecargoFormComponent implements OnInit, OnChanges, OnDestroy {
  ...
  private subscribers() {
    // Listen to import
    this.form.get('tipoDeCalculo').valueChanges
    .takeUntil(this.destroy$)
    .subscribe( importe => {
      this.actualizarTotal();
    });

    this.form.get('cargo').valueChanges
    .takeUntil(this.destroy$)
    .subscribe( importe => {
      this.actualizarTotal();
    });
  ...
  private actualizarTotal() {
    ...
    if (tipo === 'PORCENTAJE') {
      ...
      if (cargo > 0.0) {
        const saldoTotal = _.sumBy(this.partidas.value, (item: any) => item.cuentaPorCobrar.saldo );
        const total = saldoTotal * (cargo / 100.00);
